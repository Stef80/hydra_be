INSERT INTO users (email, name, surname, password, workplace, expertise_area, actived) values
('stefano.longobucco@agmsolutions.net', 'Stefano', 'Longobucco', '12345', 'Milano', 'jave be','true');



INSERT INTO projects (name, description, start_date, end_date, total_days) values
('HYDRA_BE','gestione smartworking back-end','18-01-2021', '30-04-2021', 90);




--INSERT INTO tasks(task_name, project_id,date_of_registation, status) values
--('analisi funzionale', 1, '18-01-2021','1'),
--('analisi Base di dati', 1, '22-01-2021','1'),
--('analisi End Point', 1, '22-01-2021','1');


--INSERT INTO updates()